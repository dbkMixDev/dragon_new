...
try {
    ...
    $queryRental = "UPDATE tb_trans SET is_deleted = 1 
                    WHERE inv = ? AND userx = ? AND (is_deleted IS NULL OR is_deleted != 1)";
    ...
    $queryFnb = "UPDATE tb_trans_fnb SET is_deleted = 1 
                 WHERE inv = ? AND userx = ? AND (is_deleted IS NULL OR is_deleted != 1)";
    ...
    $querySpending = "UPDATE tb_trans_out SET is_deleted = 1 , userdel = ? , datedel = ?
                      WHERE invoice = ? AND userx = ? AND (is_deleted IS NULL OR is_deleted != 1)";
    $stmtSpending = mysqli_prepare($con, $querySpending);
    mysqli_stmt_bind_param($stmtSpending, "ssss", $userx,$dateTime,$invoice, $userx);
    ...
    $queryFinal = "UPDATE tb_trans_final SET is_deleted = 1 , userdel = ? , datedel = ?
                   WHERE invoice = ? AND userx = ? AND (is_deleted IS NULL OR is_deleted != 1) ";
    $stmtFinal = mysqli_prepare($con, $queryFinal);
    mysqli_stmt_bind_param($stmtFinal, "ssss", $userx,$dateTime,$invoice, $userx);
