...
<button 
  ...
  <div class="page-title-box d-sm-flex align-items-center justify-content-between">
    <h4 class="mb-sm-0 font-size-18">Transactions List</h4>
    ...
    <button type="button" class="btn btn-success btn-sm position-relative"
        ...
        data-bs-target="#modal<?= $psnumberx ?>"
        style="width: 100%;"
       >
 ...
 <button type="button" class="btn btn-danger btn-sm position-relative"
        data-bs-toggle="modal"
        data-bs-target="#modalSpending<?= $psnumberx ?>"
        style="width: 100%;">
...
$(document).ready(function() {
    ...
    transactionTable = $('#datatable-buttons').DataTable({
        ...
        ajax: {
            url: 'controller/fetch_transactions.php',
            type: 'GET',
            data: function(d) {
...

{
  ...
  render: function(data, type, row) {
    if (row.is_parent) {
        ...
        if (row.kategori !== 'Spending') {
            printButton = `
                <a href="#"
                   onclick="window.open('controller/print_struk.php?inv=${row.inv}', 'strukWindow', 'width=500,height=800,top=100,left=300'); return false;"
                   target="_blank"
            ...
            downloadButton = `
                <a href="#"
                   onclick="window.open('controller/download_struk.php?inv=${row.inv}', 'strukWindow', 'width=500,height=800,top=100,left=300'); return false;"
                   target="_blank"
                   class="btn btn-sm text-white"
                   style="background-color: #00809D;"
...
$('#datatable-buttons').on('click', '.btn-delete-transaction', function() {
    ...
    Swal.fire({
        title: 'Yakin ingin menghapus seluruh invoice?',
        html: `<strong>Invoice: ${invoice}</strong><br>
               Semua transaksi dalam invoice ini akan dihapus:<br>
        ...
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: 'Ya, hapus seluruh invoice!',
        cancelButtonText: 'Batal',
        reverseButtons: true,
        ...
        if (result.isConfirmed) {
            ...
            Swal.fire({
                title: 'Menghapus...',
                text: 'Sedang menghapus semua transaksi dalam invoice',
                ...
                success: function(response) {
                    if (response.success) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Berhasil!',
                            html: `Invoice <strong>${invoice}</strong> berhasil dihapus.<br>
